...
public class Bracket {
    ...
    public void buildBracket(Context context, int tierWidth, int aboveViewId, int columnViewId,
        ...
        for (Integer mPos = 0; mPos < length(); mPos++) {
            ...
            if (sm1Types.get(mPos) == BrNodeType.TIP || sm1Types.get(mPos) == BrNodeType.RESPAWN) {
    ...
    public void respawnFromParentBracket(Bracket br) {
        ...
        for (int idx = 0; idx < matchIds.size() - 1; idx++) {
            ...
            Log.i(LOGTAG, "Upper. respawnId: " + respawnId + " (" + nodeType.toString() + "), " +
                    "matchIdx: " + matchIdx);
            if (matchIdx >= 0) {
            ...
            matchIdx = br.matchIds.indexOf(respawnId);
            Log.i(LOGTAG, "Lower. respawnId: " + respawnId + " (" + nodeType.toString() + "), " +
                    "matchIdx: " + matchIdx);
            if (matchIdx >= 0) {
                if (nodeType == BrNodeType.RESPAWN) {
                    if (br.sm1Types.get(matchIdx) == BrNodeType.LOSS) {
    ...
    private void makeInvisibleHeaders(int baseWidth, int tierWidth, int aboveViewId,
                                      int columnViewId) {
    ...
    public TextView makeHalfMatchView(Context context, int idx, Boolean upper) {
        ...
        if (upper) {
            ...
            if (smType == BrNodeType.TIP) {
                ...
                tv.setText("(" + String.valueOf(sm.getSeed() + 1) + ") " + sm.getTeam().getName());
                drwStr += "_labeled";
    ...
    public void refreshViews() {
        ...
        for (int idx = 0; idx < length(); idx++) {
            ...
            if (smType != BrNodeType.NA) {
                ...
                if (smType != BrNodeType.UNSET && smType != BrNodeType.RESPAWN) {
                    drwColor = smSeedMap.get(sm2Idcs.get(idx)).getTeam().getColor();
                }
                if (smType == BrNodeType.LOSS) {
                    drwString += "_eliminated";
                }
    ...
    private int getTier(int viewId) {
        ...
        return ((Double) Math.floor(-Math.log(1 - ((double) matchId) / nLeafs) / Math.log(2)))
                .intValue();
    }
    ...
    private void byeByes() {
        ...
        for (int ii = 0; ii < matchIds.size(); ii++) {
            if (sm1Types.get(ii) == BrNodeType.BYE || sm2Types.get(ii) == BrNodeType.BYE) {
    ...
    private Boolean hasSm(int idx, int smIdx) {
        Boolean hasSm = false;
        if (sm1Idcs.get(idx) == smIdx && sm1Types.get(idx) != BrNodeType.RESPAWN) {
            hasSm = true;
        } else if (sm2Idcs.get(idx) == smIdx && sm2Types.get(idx) != BrNodeType.RESPAWN) {
            hasSm = true;
        }
        return hasSm;
    }
    ...
    public MatchInfo getMatchInfo(int viewId) {
        ...
        if (matchIds.contains(matchId)) {
            ...
            if (sm2Type == BrNodeType.UNSET || sm2Type == BrNodeType.RESPAWN) {
                ...
                title += " -vs- " + smSeedMap.get(sm2Idcs.get(idx)).getTeam().getName();
            ...
            String subtitle = "id: " + id_in_session + ", match: " + matchIds.get(idx) +
                    ", gameId: " + game_id;
    ...
    private void logMatchList(String header) {
        Log.i(LOGTAG, header);
        for (int ii = 0; ii < matchIds.size(); ii++) {
            Log.i(LOGTAG, "(" + ii + ") Match " + matchIds.get(ii) + ", p1Seed: " +
                    sm1Idcs.get(ii) + " (" + sm1Types.get(ii).toString() + "), p2Seed: " +
                    sm2Idcs.get(ii) + " (" + sm2Types.get(ii).toString() + "), gId: " +
                    gameIds.get(ii));
        }
    }
}
