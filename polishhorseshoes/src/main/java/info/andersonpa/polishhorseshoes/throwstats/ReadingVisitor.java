package info.andersonpa.polishhorseshoes.throwstats;
...
public class ReadingVisitor extends BaseWalkingVisitor implements Iterator<String>,
    ...
    String render_line(int idx) {
        ...
        s = lineage +
                "  " + counts.get(idx) + ",   " + String.format("%.1f",
                100 * conditionalFractions.get(idx)) + "+/-" + String.format("%.1f",
                100 * conditionalFractionStdDev.get(idx)) + "%" + ",   " + String.format("%.2f",
                100 * absoluteFractions.get(idx)) + "+/-" + String.format("%.2f",
                100 * absoluteFractionStdDev.get(idx)) + "%";
